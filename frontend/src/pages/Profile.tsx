...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserPosts = async () => {
      if (!user) return;
      try {
        // Fetch all posts and filter by user (MVP)
        // Ideally backend should have /users/me/posts or similar
        const response = await postService.getAll(1, 100); // Fetch enough to filter
        if (response.success) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        // Reload page to reflect changes or update context (Context updates automatically via localStorage in this simplified setup, but state might need refresh)
